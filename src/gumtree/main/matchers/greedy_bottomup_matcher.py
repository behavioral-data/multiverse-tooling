    ...
    def match(self, src: Tree, dst: Tree, mappings: MappingStore) -> MappingStore:
        for t in src.post_order(): # line 1 in alg 2
            if t.is_root():
                ...
                break
            elif not(mappings.is_src_mapped(t) or t.is_leaf()): # t1 is not matched and t1 has children
                ...
                if best is not None:
                    # self.last_change_match(mappings, t, dst)
                    mappings.add_mapping(t, best)
                    
        return mappings

# TODO implement python tests to test a lot of the modules